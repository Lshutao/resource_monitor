...
(function() {
	XUI.tip.init = function(panel) {// 鼠标提示
		...
		$(content).find("*[rel=popover]").each(function(i, o) {
			var contentEl = $(o).attr("data-content"), contentFunc = null;
			if (!XUI.isne(contentEl) && contentEl.indexOf("javascript:") == 0) {
				$(o).removeAttr("data-content");
				contentFunc = function() {
					var text = eval(contentEl.substr(11));
					$(o).attr("data-content", text);
					return text;
				};
			}
			$(o).removeAttr("rel").popover({
				content : contentFunc
			});
		});
		$(content).find("*[rel=tooltip]").each(function(i, o) {
			$(o).removeAttr("rel").tooltip();
		});
		$(content).find("*.tip").each(function(i, o) {
			$(o).removeClass("tip").attr("title", $(o).text().trim());
		});
	};
		...
		$(table)
				...
				.bind(
						...
						function(ev) {
							...
							var effect = (ev.clientX - left) < resizeWidth
									|| (th.outerWidth() - (ev.clientX - left)) < resizeWidth;
							if (effect) {
								th.css('cursor', 'col-resize');
							} else {
								th.css('cursor', oldCursor);
							}
						})
				.unbind("mousedown")
				.bind(
						...
						function(ev) {
							...
							var effect = (ev.clientX - pos.left) < resizeWidth
									|| (dd.obj.outerWidth() - (ev.clientX - pos.left)) < resizeWidth;
							if (!effect || dd.obj.prevAll().length < 1
									|| dd.obj.nextAll().length < 1) {
								return;
							}// 跳过第一列和最后一列
							dd.line = $("<div class='dragLine'></div>");// 竖线
							dd.line.css({
								"height" : dd.table.height(),
								"top" : dd.table.offset().top,
								"left" : ev.clientX,
								"display" : "block"
							});// 显示调整线
							if (ev.clientX - pos.left < dd.obj.width() / 2) {
								dd.obj = dd.obj.prev();
							}// 记录当前TH
							var x = ev.clientX;
							dd.rangeStart = dd.table.offset().left;
							dd.rangeEnd = dd.rangeStart + dd.table.width();
							$("body")
									.append(dd.line)
									...
									.bind(
											"mousemove",
											function(event) {
												if (dd.rangeStart < event.clientX
														&& dd.rangeEnd > event.clientX) {
													dd.line.css({
														"left" : event.clientX
													});
													dd.deltaX = event.clientX
															- x;
												}
											})
									.bind(
											...
											function(event) {
												dd.line.remove();
												dd.obj.width(Math.max(40,
														dd.obj.width()
																+ dd.deltaX));
												var defaultWidth = parseInt(dd.table
														.attr("defaultWidth"));
												dd.table.width(Math.max(
														defaultWidth, dd.table
																.width()
																+ dd.deltaX));
	...
	XUI.table.sorting = function(panel) {
		...
		$(sorting)
				...
				.bind(
						"click",
						function() {
							var field = $(this).attr("data-orderField"), type = $(
									this).attr("data-orderFieldType");
							var form = XUI.vmContainer().find("FORM")[0];
							if (form["orderField"] == null) {
								$(form)
										.append(
												"<input type='hidden' name='orderField'/>");
							}
							if (form["orderFieldType"] == null) {
								$(form)
										.append(
												"<input type='hidden' name='orderFieldType'/>");
							}
...
(function() {
	...
	XUI.window.open = function(url, params, method, conf) {
		var open = function(data) {
			// JSON数据
			try {
				if (data != null && !XUI.isne(data.result)) {
					alert(data.result);
					return;
				}
			} catch (e) {
			...
			modal.attr("data-url", url).attr("data-params", params).attr(
					"data-method", method);
		};

		XUI.ajax.send(url, params, method, null, {
			callback : open
		});
	};
	...
	XUI.window.openDialog = function(content, conf) {
		...
		var modalFooter = "";
		modalBody.find("DIV.bottomBar>button,DIV.bottomBar>A").each(
				function(i, o) {
		...
		modal.append(modalHeader).append(modalBody).append(modalFooter)
				.appendTo(document.body).before(backdrop);
		setTimeout(function() {
			modal.css({
				"z-index" : zIndex + 1,
				"width" : options.width,
	...
	XUI.window.close = function() {
		...
		if (length > 0) {
			XUI.window.queue[length - 1].find('DIV.modal-header>A.close')
					.click();
		}
	};
	// 关闭所有窗口
	...
	XUI.window.refresh = function() {// 刷新顶层窗口
		if (XUI.window.queue.length <= 0) {
			return false;
		}
		var modal = XUI.window.queue[XUI.window.queue.length - 1];
		var opts = {
			"url" : modal.attr("data-url"),
			"params" : modal.attr("data-params"),
		...
		XUI.ajax.send(opts.url, opts.params, opts.method, null, {
			callback : refresh
		});
		return true;
	};
...
(function() {
	XUI.form.page = function(num) {
		...
		if (form["currentPage"] == null) {
			$(form).append("<input type='hidden' name='currentPage'/>");
		}
		...
		$(from)
				...
				.bind(
						...
						function() {// 阻止表单默认提交事件
							...
							var isJSON = (!XUI.isne(dataType) && dataType
									.toUpperCase() == "JSON");
							var refresh = ($(this).attr("refresh") == "true" ? true
									: false);
							XUI.ajax.send($(this).attr("action"), $(this)
									.serialize(), $(this).attr("method"),
									dataType, {
										"closeWindow" : isJSON || refresh,
										"refresh" : isJSON || refresh
									});
							return false;
						});
	};
	XUI.form.pageNumber = function(){
		var amount_value = $("#page-pageAmount").attr("value");
		var num_value = $("#page-number").attr("value");
		if( num_value > amount_value){
			return false;
		}else{
			XUI.form.page(num_value);
		}
	}; 
...
(function() {
	...
	var successMethod = function(data, dataType) {//请求成功
		...
		var message = {
			"code" : "failure",
			"result" : "请求URL[" + url + "],发生" + xhr["statusText"] + "错误."
		};
		var dataType = XUI.isne(requestType) ? "" : requestType.toUpperCase();
		switch (xhr["status"]) {
		case 403:
			message["result"] = "您无权访问本页面.";
			break;
		case 404:
			message["result"] = "不存在的URL[" + url + "]";
			break;
		case 500:
			message["result"] = "服务器内部错误." + xhr["statusText"];
			break;
		}
		if (dataType == "JSON"
				|| (!XUI.isne(url) && url.toUpperCase().endsWith(".JSON"))) {
			successMethod(message, dataType);
		} else {
			successMethod("<div class='alert'>" + message["result"] + "</div>",
					dataType);
		}
	...
	XUI.refreshVM = function() {
		XUI.ajax.send(vmData.url, vmData.params, vmData.method);
		return true;
	};
	...
	XUI.ajax.send = function(url, params, method, type, config) {
		...
		var ajaxOptions = {
			...
			success : function(data) {
				var type = XUI.isne(this.dataType) ? "" : this.dataType
						.toUpperCase();
				...
				if (type == "JSON" && data.code == "failure") {
					return;
				}
	...
	//以JSON的方式执行操作
	XUI.ajax.action = function(url, params, config, tip){
		var cfg = $.extend(true, {}, {
			"confirm" : tip
		}, config || {});
		if (window.confirm(cfg.confirm)) {
			XUI.ajax.send(url, params, "GET", "", config);
		}
	};
	...
	//请求时界面遮罩
	XUI.ajax.mask = function(msg) {
		var info = msg;
		if ($("#winModal,#loadInfo").length == 0) {
			var msg = (!XUI.isne(info)) ? info : "正在努力为您获取数据,请稍候...";
			$("body")
					.append(
							"<div id='winModal' class='modal-backdrop fade in' style='z-index:9999'></div><div id='loadInfo'>"
									+ msg + "</div>");
		}
	};
	XUI.ajax.unmask = function() {
	...
	/*jQuery.ajaxSetup({
		...
		error : function(xhr, ts, et) {
			errorMethod(xhr, this.url, this.dataType);
			this.complete(xhr, ts);
		},
